...
public class AnimationManager {
    ...
    public void render_zombie(SpriteBatch batch) {
        ...
        if (zombieClicker.getNumerics().get_bossFight()) {                                                   //ЕСЛИ ИДЕТ БОССФАЙТ
            if (zombieClicker.getNumerics().getBoss_health().compareTo(BigInteger.valueOf(0)) <= 0) {        //УБИЙСТВО БОССА
                zombieClicker.getMainGame().next_level();
                System.out.println("boss dead");
                zombie_animation = zombie_animations_arr.get(MathUtils.random(0, 2));
            }
        } else if (zombieClicker.getNumerics().getZombie_health().compareTo(BigInteger.valueOf(0)) <= 0) {          //УБИЙСТВО ЗОМБИ
            zombieClicker.getNumerics().plus_zombie_kills(BigInteger.valueOf(1));
            zombieClicker.getNumerics().plus_gold(zombieClicker.getNumerics().getZombie_kill_reward());
